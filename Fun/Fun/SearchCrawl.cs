...
namespace Fun
{
    ...
    k => char.ToString(
        ...
        public static string SearchLinks(string text, bool random)
        {
            ...
            var list = matches.Cast<Match>().Where(m => m.Groups.Count >= 2 && !m.Groups[1].Value.Contains("webcache.googleusercontent.com") && !m.Groups[1].Value.Contains("google.com")).Select(m => m.Groups[1].ToString()).ToList();
        ...
        {
            if (im)
                return string.Format("https://google.com/search?hl=en&tbm=isch&q={0}{1}", WebUtility.UrlEncode(text), gif ? "&tbs=itp:animated" : "");
            else
                return string.Format("https://google.com/search?hl=en&q={0}", WebUtility.UrlEncode(text));
        }
